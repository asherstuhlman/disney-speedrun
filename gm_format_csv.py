...
def format_csv_for_predictions(prediction_time_delta = 1):
    ...
    y_df = pd.read_csv('http://stuhlman.net/gminus/js/ride_data_'+str(yesterday.month)+'-'+str(yesterday.day)+'.csv', encoding = "ISO-8859-1")
    out_df = pd.read_csv('http://stuhlman.net/gminus/js/rides_and_ids.csv', encoding = "ISO-8859-1").sort_values(by=['id'])
