...
def main():
    for do_192_times in range(192):
        ...
        if int(last_updated) != now.day:
            ...
            save_js_remotely('ride_data_cleaned_'+str(yesterday.month)+'-'+str(yesterday.day)+'.csv',gm_format_csv.format_csv_for_predictions(1))
        ...
        if (do_192_times<191):
            ...
            if (datetime.now(pytz.timezone("US/Pacific")) - now).seconds < 300:
